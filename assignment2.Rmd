...
model_fd1
```
The third model is a first difference model with time trend and without lags.
...
model_fd2
```
The fourth model is also a first difference model with time trend, but this time with 2 years of lag.
...
model_fd3
```
The fifth model is the last of the first difference models, with 6 years of lag this time.
...
The sixth model is a fixed effects model.

```{r model long diff, echo=F}
#creating new table with values only from 1992 and 2018
data_long_diff <- data %>% filter((year == 2018)|(year == 1992)) %>% 
  pivot_wider(
    names_from = year,
